...
def valid_index(argument):
    if not argument.isdigit():
        raise commands.BadArgument("Index must be a number.")
    return int(argument)
...
class Profile(commands.Cog):
    ...
    async def get_profile(self, ctx, *, member=None, index=None):
        ...
        if index is not None:
            ...
            try:
                ...
                raise commands.BadArgument("Invalid index.") from None
        else:
            query = """SELECT profile.id, platform, username
                       FROM profile
                       INNER JOIN member
    ...
    async def set_or_remove_nickname(self, ctx, *, profile=None, profile_id=None, remove=False):
        member = ctx.author
        ...
        try:
            ...
            await ctx.send("Something bad happened while updating your nickname.")
        ...
        if not remove:
            ...
            await ctx.send("Nickname successfully set.")
        else:
    ...
    @has_profile()
    @commands.group(invoke_without_command=True, brief="Lists your profiles.")
    ...
    @can_add_profile()
    @profile.command(aliases=["add"], brief="Link an Overwatch profile.")
    async def link(self, ctx):
        """Link an Overwatch profile to your Discord account."""
        view = discord.ui.View()
        view.add_item(DropdownPlatform())
        await ctx.send("Link a profile", view=view)
        await view.wait()

        platform = view.children[0].platform
    ...
    @has_profile()
    @profile.command(aliases=["remove"], brief="Unlink a profile.")
    async def unlink(self, ctx, index: valid_index):
        ...
        if await self.is_main_profile(ctx.author.id, profile_id=id_) and len(profiles) > 1:
            message = (
                "You can't unlink the main profile if multiple profiles are linked. "
    ...
    @has_profile()
    @profile.command(brief="Update a profile.")
    ...
    @has_profile()
    @profile.command(brief="Update the main profile.")
    ...
    @has_profile()
    @profile.command(aliases=["rank", "sr"], brief="Provides SRs information for a profile.")
    async def rating(self, ctx, index: valid_index = None, member: discord.Member = None):
        ...
        async with ctx.typing():
            member = member or ctx.author
            id_, platform, username = await self.get_profile(ctx, member=member, index=index)
    ...
    @has_profile()
    @profile.command(aliases=["statistics"], brief="Provides general stats for a profile.")
    async def stats(self, ctx, index: valid_index = None, member: discord.Member = None):
        ...
        async with ctx.typing():
            member = member or ctx.author
            unused, platform, username = await self.get_profile(ctx, member=member, index=index)
    ...
    @has_profile()
    @profile.command(brief="Provides general hero stats for a profile.")
    async def hero(self, ctx, hero: Hero, index: valid_index = None, member: discord.Member = None):
    ...
    @has_profile()
    @profile.command(aliases=["nick"], brief="Shows your SRs in your nickname.")
    @commands.guild_only()
    ...
    @is_premium()
    @has_profile()
    @profile.command(brief="`[Premium]` Shows SRs performance graph.")
