...
class HelpPaginator(Base):
    ...
    async def show_help(self, payload):
        ...
        embed.set_footer(text="Press one of the arrows to go back.")
...
async def get_bot_help_pages(ctx, commands):
    ...
    for index, page in enumerate(pages, start=1):
        embed = discord.Embed(color=main_color)
        embed.title = "{bot} Command List".format(bot=ctx.bot.user.display_name)
...
async def get_group_help_pages(ctx, help_command, group, commands):
    ...
    for index, page in enumerate(pages, start=1):
        ...
        for command in page:
            ...
            value = command.brief or "No help found..."
            ...
            embed.set_footer(
                text='Use "{prefix}help [command]" for more info on a command.'.format(
                    prefix=help_command.clean_prefix
                )
            )
        all_pages.append(embed)
    return all_pages
